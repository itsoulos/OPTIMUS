...
string  ClassProgram::printF(vector<double> &genome)
{

    string ret="";
    if(dpgenome.size()!=genome.size()/(nclass-1))
        dpgenome.resize(genome.size()/(nclass-1));
    char str[1024];
    for(int i=0;i<nclass-1;i++)
    {
        for(int j=0;j<genome.size()/(nclass-1);j++)
        {
            dpgenome[j]=genome[i*genome.size()/(nclass-1)+j];
        }
        int redo=0;
        pstring[i]=printRandomProgram(dpgenome,redo);
...
double	ClassProgram::getClassError(vector<double> &genome,QVector<Data> &x,Data &y)
{
    double X[problem_dimension];
    double value=0.0;
    if(dpgenome.size()!=genome.size()/(nclass-1))
    ...
    for(int i=0;i<nclass-1;i++)
    {
        for(int j=0;j<dpgenome.size();j++)
            dpgenome[j]=genome[i*genome.size()/(nclass-1)+j];
        int redo=0;
        string s = printRandomProgram(dpgenome,redo);
        if(redo>=REDO_MAX) return -1e+8;
        pstring[i]=s;
    }
    for(int j=0;j<nclass-1;j++)
...
double 	ClassProgram::fitness(vector<double> &genome)
{
        double *X=new double[problem_dimension];
    double value=0.0;
    if(dpgenome.size()!=genome.size()/(nclass-1))
        dpgenome.resize(genome.size()/(nclass-1));
    if(outy.size()!=trainy.size()) outy.resize(trainy.size());
    for(int i=0;i<(int)outy.size();i++) outy[i]=NAN_CLASS;
    for(int i=0;i<nclass-1;i++)
    {
        for(int j=0;j<(int)dpgenome.size();j++)
        {
            int index=i * genome.size()/(nclass-1)+j;
            dpgenome[j]=genome[index];
        }
        int redo=0;
        string s = printRandomProgram(dpgenome,redo);
        if(redo>=REDO_MAX) {delete[] X;return -1e+8;}
