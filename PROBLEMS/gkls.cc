...
void    init(QJsonObject data)
{
	...
	if(data.contains("gkls_dimension")) 
		gkls_dimension=data["gkls_dimension"].toString().toInt();
		GKLS_dim = gkls_dimension;
	if(data.contains("gkls_minima"))
		gkls_minima=data["gkls_minima"].toString().toInt();
		GKLS_num_minima = gkls_minima;
		GKLS_domain_alloc();
		GKLS_global_dist=2.0/3.0;
		GKLS_global_radius=0.5*GKLS_global_dist;
		GKLS_global_value=GKLS_GLOBAL_MIN_VALUE;
	        GKLS_parameters_check();
	if(data.contains("gkls_function"))
		gkls_function=data["gkls_function"].toString().toInt();
		int func_num=gkls_function;
