...
double tolmin(Data  &x,MinInfo &Info)
{
    ...
    for(int i=0;i<n;i++)
    {
		...
		fmin=Info.problem->getTrainError(x);
		...
		MinInfo &info)
{
	...
	*f=info.problem->getTrainError(Xa);
	...
	for(int i=0;i<*n;i++)
	{
		double eps=pow(1e-18,1.0/3.0)*dmax(1.0,fabs(Xa[i]));
		Xa[i]+=eps;
		double v1=info.problem->getTrainError(Xa);
		Xa[i]-=2.0 *eps;
		double v2=info.problem->getTrainError(Xa);
		g[i]=(v1-v2)/(2.0 * eps);
		Xa[i]+=eps;
	}
	double v=0.0;
