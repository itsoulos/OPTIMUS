...
then
	METHODPARAMS="--localsearch_rate=0.05 --localsearch_method=bfgs"
elif [ $METHOD = "Pso" ]
then
...
./OptimusApp --filename=../PROBLEMS/lib$PROBLEM.so  --opt_method=$METHOD   $METHODPARAMS  $MLPARAMS --interval_generations=50 --threads=1  --iterations=30
