...
then
	METHODPARAMS="--double_stoprule=stoponzero --double_localsearchrate=0.05 --localsearch_method=bfgs --bfgs_debug=no --double_generations=1000"	
elif [ $METHOD = "Genetic" ]
then
...
PROBLEM=$1
MPARAMS="--model=mlp --weights=10 --lambda=10 --npoints=100"
