...
then
	METHODPARAMS="--chromosome_count=200 --max_generations=500 --localsearch_method=grs --localsearch_rate=0.005"
elif [ $METHOD = "Pso" ]
then
...
#echo ./OptimusApp --filename=../PROBLEMS/$PROBLEM  --opt_method=$METHOD   $METHODPARAMS  $MLPARAMS --threads=12  --iterations=30
 ./OptimusApp --filename=../PROBLEMS/$PROBLEM  --opt_method=$METHOD   $METHODPARAMS  $MLPARAMS --threads=8  --iterations=10
