...
void printParams()
{
    ...
    printOption("--boundMethod","The  optimization method used in the bound phase",boundMethod);
    printOption("--trainMethod","The optimization method used in the train phase",trainMethod);
...
void parseCmdLine(QStringList args)
{
    ...
    for(int i=1;i<args.size();i++)
    {
        ...
        bool b6=checkParameter(name,value,"--boundMethod","-b",boundMethod);
        bool b8=checkParameter(name,value,"--trainMethod","-a",trainMethod);
...
int main(int argc, char *argv[])
{
    ...
    else
    {
       ...
       //now run the bound construction phase
       fitProblem->setPhase(BOUNDWEIGHT_PHASE,bestx);
       optimizerList[boundMethod]->Solve();
       //now run the train phase
       //get the margings of the first phase and set it
       bestx=mainProblem->getBestx();
       fitProblem->setPhase(TRAIN_PHASE,bestx);
