...
\citep{merlin} which is accompanied with the Merlin Control Language
...
as the Paradiseo \citep{paradiseo} implemented in C++ that mainly
contains evolutionary algorithms, the Pagmo software \citep{pagmo}
...
used in areas such as data mining applications \citep{de_datamining,de_symmetry1},
material design problems \citep{de_symmetry3}, feature selection
\citep{de_symmetry6}, clustering methods \citep{de_symmetry7} etc.
...
For example, for the Lennard Jones potential \citep{originalJones},
the user can pass the number of atoms in the potential using an assignment
as follows\\
\inputencoding{latin9}\begin{lstlisting}
        natoms=data["natoms"].toString().toInt(); 
\end{lstlisting}
\inputencoding{utf8}If the objective problem does not need a parameter, then this function
can be empty.
...
double precision intervals eg the value $[2,4${]} is an interval
with left bound the value 2 and right bound the value 4.
...
point $x$. The number of elements of the vector x must be the same
in number as the return value of the function getDimension().
...
$f(x)$. This function can be used in various cases, such as to generate
a graph after the optimization is finished or even in the case of
artificial neural networks \citep{nn1,nn2} to apply the resulting
trained network to the test set of the problem.
\end{enumerate}
\begin{figure}
...
\hline 
\textbf{ATOMS} & \textbf{GENETIC} & \textbf{PESO} & \textbf{NEURALMINIMIZER}\tabularnewline
\hline 
...
solutions of differential equations \citep{rbfde1,rbfde2}, issues
of communication \citep{rbfnetwork1,rbfnetwork2},\textbf{ }problems
...
are calculated using the well - known K-Means algorithm \citep{kmeans}.
...
\citep{tsoulosAI,tsoulosQFC}. The comparative results are shown in
Table \ref{tab:Experimental-results}. The RBF network was implemented
in ANSI C++ using the freely available Armadillo library \citep{Armadillo}.
...
problem has many applications in areas such as as physics \citep{ConPhysics},
astronomy \citep{ConAstro}, chemistry \citep{ConChem}, biology \citep{ConBio}
etc. Also, a variety of methods have been proposed for problems of
this category, such as as Lagrange multiplier methods \citep{cons1},
Trust region methods \citep{cons2}, interval methods \citep{cons3},
differential evolution \citep{cons4} etc.\textbf{ }Constrained optimization
...
\begin{itemize}
\item \textbf{Levy} function\citep{Levy}. The function is defined as: 
\[
...
\]
with $a=2,\ b=0.25$. 
\item \textbf{Salkin} function\citep{Salkin}. The objective function is
defined as: 
\[
...
\end{eqnarray*}
\item \textbf{Hess} function\citep{Hess}. The function is defined as:
\[
...
\end{eqnarray*}
\item \textbf{Chootinan1} function\citep{choot}. The objective problem
is defined as:
\[
...
\end{eqnarray*}
\item \textbf{G15} function\citep{G15}. The function is defined as:
\[
