...
\abstract{A variety of problems from many research areas can be modeled using
...
The main features of the proposed software are: a) Ability to code
the objective problem in a high level language such as ANSI C++ b)
Incorporation of many global optimization techniques to solve the
objective problem c)Parameterization of global optimization methods
using user-defined parameters.}


\keyword{Global optimization; stochastic methods; termination rules.}
...
\end{lstlisting}
\inputencoding{utf8}\item \textbf{int} getDimension(). This function returns the dimension of
...
of the objective problem. The class Interval is a simple class that
represents double precision intervals eg $[2,4${]} is an interval
...
\end{lstlisting}
\inputencoding{utf8}The command line arguments have as follows:
\begin{enumerate}
