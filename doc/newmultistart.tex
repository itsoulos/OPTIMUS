...
This article proposes novel method based on the Multistart, that utilizes
a mechanism to prevent unnecessary local optimization calls as well
as an asymptotic stopping rule. The proposed method has been evaluated
on a series of global optimization functions from the relevant literature
and it is compared against some other optimization methods and the
...
every iteration a number of samples is taken in the bounding box of
the objective problem. Some of them are considered as starting points
for a local search procedure and the rest are discarded. The method
continues until the maximum number of iterations is reached or an
asymptotic termination rule is satisfied. The main steps of the proposed
algorithm are outlined in Algorithm \ref{alg:The-main-steps}. In
the following subsection the main parts of the proposed algorithm
...
The proposed method can be used also in constrained optimization problems,
...
optimization functions that are listed in the Appendix. The method
was tested against four methods from the relevant literature:
\begin{enumerate}
\item Multistart method.
\item Controlled Random Search\cite{crs3} denoted as CRS in the results.
\item Simulated Annealing\cite{simann2}, denoted as SA in the results.
\item Particle swarm optimization, denoted as PSO in the results.
\end{enumerate}
...
on bigger problems such Potential20, a problem with 60 dimensions
and thousands of local minima. On the other hand, the Simulated Annealing
