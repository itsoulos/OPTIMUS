...
Optimization and, in the second phase, the network parameters are
trained within the bounds of the first phase using global optimization
techniques. The proposed technique was applied to a number of well-known
...
decades in a variety of practical problems from many different fields
such as physics problems \citep{nnphysics1,nnphysics2,nnphysics3},
chemistry problems \citep{nnchem1,nnchem2,nnchem3}, problems related
to medicine \citep{nnmed1,nnmed2}, economic problems \citep{nnecon1,nnecon2,nnecon3}
...
as a function $N(\overrightarrow{x},\overrightarrow{w})$, where the
vector $\overrightarrow{x}$ is the input pattern to the network and
the vector $\overrightarrow{w}$ is called the weight vector. To estimate
the weight vector, the so-called training error is minimized, which
...
In addition, many researchers were also concerned with the construction
of artificial neural network architectures, such as the usage of Cross
Validation to construct the architecture of neural networks \citep{nncross},
...
sets. The overfitting problem is usually tackled through a variety
...
the proposed method is fully analyzed and discussed, in section \ref{sec:Experiments}
the experimental datasets as well as the experimental results are
listed and discussed and finally in section \ref{sec:Conclusions}
...
Consider a neural network with one processing level that uses the
so - called sigmoid function as activation function. The sigmoid function
is defined as:
\begin{equation}
\sigma(x)=\frac{1}{1+\exp(-x)}\label{eq:sig}
\end{equation}
This function is graphically illustrated in Figure \ref{fig:sigPlot}.

\begin{figure}[H]
\includegraphics{sig}

\caption{Plot of the sigmoid function.\label{fig:sigPlot}}
...
\end{equation}
where the vector $w_{i}$ stands for the the weight vector and the
value $\theta_{i}$ denotes the bias the node $i$. The equation for
a neural network with $H$ hidden is defined as:
\begin{equation}
N(x)=\sum_{i=1}^{H}v_{i}o_{i}(x),\label{eq:eq1-2}
\end{equation}
The value $v_{i}$ denotes the output weight for node $i$. Therefore
writing the overall equation of the artificial neural network using
equations \ref{eq:eq1-1} and \ref{eq:eq1-2} has as:
\begin{equation}
N\left(\overrightarrow{x},\overrightarrow{w}\right)=\sum_{i=1}^{H}w_{(d+2)i-(d+1)}\sigma\left(\sum_{j=1}^{d}x_{j}w_{(d+2)i-(d+1)+j}+w_{(d+2)i}\right)\label{eq:nn}
\end{equation}
The value $d$ stands for the dimension of the input vector $\overrightarrow{x}$.
Observing the equation \ref{eq:nn} but also the shape of the sigmoid
function in figure \ref{fig:sigPlot}, it is obvious that in many
cases, the sigmoid function is driven to 1 or 0 and as a consequence
the training error of the neural network can get trapped in local
minima and finally, the neural network it will lose its generalization
abilities. Therefore, a technique should be devised by which the values
of the sigmoid will be restricted to some interval of values. In the
present work, the limitation of the neural network parameters to a
range of values is carried out using the Particle Swarm Optimization
method.
...
\textbf{}
\begin{algorithm}[H]
\textbf{\caption{Calculation of the quantity $B(L)$ for a given parameter $L.$ The
parameter $M$ stands for the number of patterns for the neural network
...
The Particle Swarm Optimization method is based on a population of
vectors that are also called particles. These particles are also possible
global minima of the objective function. Each particle is associated
with two vectors: the current position denoted as\textbf{ $\overrightarrow{p}$}
and the corresponding speed \textbf{$\overrightarrow{u}$} at which
they are moving towards the global minimum. In addition, each particle
maintains in the vector $p_{i,b}$ the best position in which it has
been so far and the total population, maintains in the vector $p_{\mbox{best}}$
the best position that any of the particles have found in the past.
The purpose of the method is to move the total population toward the
global minimum through a series of iterations. In each iteration,
the velocity of each particle is calculated based on its current position,
its best position in the past and the optimal position of the population. 

In the present work, the PSO technique is used to train artificial
neural networks by minimizing the error function as defined in the
equation \ref{eq:eq1-1} along with a penalty factor depending on
the function $B(L)$ defined in subsection \ref{subsec:The-bounding-algorithm}.
...
\item \textbf{Set} $H$ the number of hidden nodes for the neural network.
...
The velocity of each particle $p_{i}$ usually is calculated as:
\begin{equation}
...
network is calculated through the following procedure:
\begin{enumerate}
...
algorithm, the BFGS variant of Powell \citep{Powell} was used as
the local search procedure, which is a version of the BFGS local optimization
procedure \citep{bfgs2}, that utilizes bounds for the objective function. 

\section{Experiments \label{sec:Experiments}}

The proposed method was applied to a number of well-known problems
in the literature and compared with other well-known artificial neural
network training techniques. In addition, experiments were carried
out to show the dependence of the method on its basic parameters.
The classification datasets incorporated in the relevant experiments
can be found at:
\begin{enumerate}
...
\item \textbf{Ionosphere} dataset, found in the Johns Hopkins Ionosphere
database and it has been thoroughly studied in many papers \citep{ion1,ion2}.
...
\item \textbf{Parkinsons} dataset \citep{parkinsons}, a medical dataset
related to Parkinson's decease.
\item \textbf{Pima} dataset \citep{pima}, a medical dataset..
\item \textbf{Popfailures} dataset \citep{popfailures}, that is related
...
\item \textbf{Wdbc} dataset \citep{wdbc}, a medical dataset about breast
tumors. 
...
of abalone from various measurements. 
\item \textbf{Airfoil }dataset, a dataset used by NASA \citep{airfoil}. 
\item \textbf{Baseball} dataset, used to estimate the salary of baseball
players.
\item \textbf{BK} dataset \citep{Stat}, a dataset used to predict the points
in a basketball game. 
\item \textbf{BL} dataset, related to a experiment on the affects of machine
adjustments on the time to count bolts. 
\item \textbf{Concrete} dataset \citep{concrete}, a civil engineering dataset.
\item \textbf{Dee} dataset, used to predict the price of the electricity
energy in Spain.
...
\item \textbf{MORTGAGE} dataset, related to economic data information of
USA.
...
In order to validate the efficiency of the proposed method, the ten
- fold validation method was used and and 30 experiments were conducted
using different seeds for the random generator each time. The average
...
freely available from \url{https://github.com/kthohr/optim}. 
\item The Rprop method \citep{rpropnn} as provided by the FCNN programming
package \citep{fcn}. 
...
downloaded from \url{https://github.com/BiagioFesta/EvolutionNet}. 
\end{enumerate}
The results for the classification datasets are listed in Table \ref{tab:exp1}
and the results for the regression datasets in Table \ref{tab:exp2}.
For every table, an additional row was added with the title AVERAGE
indicating at the end showing the average classification or regression
error for all datasets. All the experiments were conducted on AMD
Epyc 7552 equipped with 32GB of RAM. The operating system was the
Ubuntu 20.04 operating system.

\begin{table}[H]
\caption{The values for the parameters of the proposed method.\label{tab:values}}
...
\end{table}
\begin{table}[H]
\caption{Experimental results for the classification datasets.\label{tab:exp1}}
...
\hline 
\hline 
Appendicitis & 18.10\% & 16.50\% & 16.30\% & 17.20\% & 16.97\%\tabularnewline
\hline 
Australian & 32.21\% & 35.65\% & 36.12\% & 31.98\% & 26.96\%\tabularnewline
\hline 
Balance & 8.97\% & 7.87\% & 8.81\% & 23.14\% & 7.52\%\tabularnewline
\hline 
Bands & 35.75\% & 36.25\% & 36.32\% & 34.30\% & 35.77\%\tabularnewline
\hline 
Cleveland & 51.60\% & 67.55\% & 61.41\% & 53.44\% & 48.40\%\tabularnewline
\hline 
Dermatology & 30.58\% & 26.14\% & 15.12\% & 32.43\% & 14.30\%\tabularnewline
\hline 
Hayes Roth & 56.18\% & 59.70\% & 37.46\% & 50.15\% & 36.33\%\tabularnewline
\hline 
Heart & 28.34\% & 38.53\% & 30.51\% & 39.27\% & 18.99\%\tabularnewline
\hline 
HouseVotes & 6.62\% & 7.48\% & 6.04\% & 10.89\% & 7.10\%\tabularnewline
\hline 
Ionosphere & 15.14\% & 16.64\% & 13.65\% & 19.67\% & 13.15\%\tabularnewline
\hline 
Liverdisorder & 31.11\% & 41.53\% & 40.26\% & 30.67\% & 32.07\%\tabularnewline
\hline 
Lymography & 23.26\% & 29.26\% & 24.67\% & 33.70\% & 27.05\%\tabularnewline
\hline 
Mammographic & 19.88\% & 46.25\% & 18.46\% & 22.85\% & 17.37\%\tabularnewline
\hline 
PageBlocks & 8.06\% & 7.93\% & 7.82\% & 10.22\% & 6.47\%\tabularnewline
\hline 
Parkinsons & 18.05\% & 24.06\% & 22.28\% & 18.56\% & 14.60\%\tabularnewline
\hline 
Pima & 32.19\% & 34.85\% & 34.27\% & 34.51\% & 26.34\%\tabularnewline
\hline 
Popfailures & 5.94\% & 5.18\% & 4.81\% & 7.05\% & 5.27\%\tabularnewline
\hline 
Regions2 & 29.39\% & 29.85\% & 27.53\% & 33.23\% & 26.29\%\tabularnewline
\hline 
Saheart & 34.86\% & 34.04\% & 34.90\% & 34.51\% & 32.49\%\tabularnewline
\hline 
Segment & 57.72\% & 49.75\% & 52.14\% & 66.72\% & 18.99\%\tabularnewline
\hline 
Wdbc & 8.56\% & 35.35\% & 21.57\% & 12.88\% & 6.01\%\tabularnewline
\hline 
Wine & 19.20\% & 29.40\% & 30.73\% & 25.43\% & 10.92\%\tabularnewline
\hline 
Z\_F\_S & 10.73\% & 47.81\% & 29.28\% & 38.41\% & 8.55\%\tabularnewline
\hline 
ZO\_NF\_S & 8.41\% & 47.43\% & 6.43\% & 43.75\% & 7.11\%\tabularnewline
\hline 
ZONF\_S & 2.60\% & 11.99\% & 27.27\% & 5.44\% & 2.61\%\tabularnewline
\hline 
ZOO & 16.67\% & 14.13\% & 15.47\% & 20.27\% & 5.80\%\tabularnewline
\hline 
...
neural networks for classification and regression problems was presented.
In the first phase, a widely used global optimization technique such
as the Particle Swarm Optimization was used to minimize the training
error of the artificial neural network to which a penalty factor had
been added. This penalty factor is used in order to maintain the effectiveness
...
and regression problems from the relevant literature and the experimental
...
literature, it appears that the proposed technique significantly outperforms,
especially in the case of regression~problems. In relevant experiments
...
\authorcontributions{All authors have conceived of the idea and methodology. A.T. and
I.G.T. conducted the experiments, employing several datasets and provided
the comparative experiments. D.T. and E.K. performed the statistical
