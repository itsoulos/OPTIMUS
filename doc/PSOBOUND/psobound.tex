...
Other tasks in a similar direction include for example the work of
Hwang and Ding \citep{neuralInterval1} that suggest prediction intervals
for the weights of neural networks, the work of Kasiviswanathan et
al \citep{neuralInterval2} that constructs prediction interval for
neural networks applied on rainfall runoff models, the work of Sodhi
and Chandra \citep{neuralInterval3} that propose an interval based
method for weight initialization of neural networks etc. Also, a review
for weight initialization strategies can be found in the paper of
Narkhede et al \citep{weightInitReview}.
...
network used in the experiments has one hidden layers with 10 neurons.
...
the classification data are shown in the tablee \ref{tab:exp1} and
the corresponing results for the regression datasets are shown in
...
more evident in regresion dataset, where the average gain from the
...
is measure on an additional experiment, where the local search method
for the proposed technique is replace by the ADAM optimizer. The results
for the classification datasets are shown in Table \ref{tab:exp1Adam}
...
Also, the average regression error for the proposed method when then
...
\caption{Average regression error for the proposed method using different number
