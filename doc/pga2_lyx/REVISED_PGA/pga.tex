...
In the relevant literature, two major categories of parallel genetic
algorithms appear: island genetic algorithms and cellular genetic
algorithms \citep{Tomohiro}. The island model is a Parallel Genetic
Algorithm (PGA), that manages several subpopulations on separate islands,
executing the genetic algorithm process on each island simultaneously
for a different set of solutions. Island models have been utilized
in various cases, such as molecular sequence alignment \citep{ga_island0},
the quadratic assignment problem \citep{ga_island1}, placement of
sensors/actuators in large structures \citep{ga_island2} etc. Also,
recently Tsoulos et al. proposed an implementation of an island PGA
\citep{pdoublepop}. In the case of the parallel cellular model of
genetic algorithms, solutions are organized into a grid. Various diverse
operators, such as crossover and mutation, are applied to neighboring
regions within the grid. For each solution, a descendant factor is
created, replacing its position within the birth region. The model
is flexible regarding the structure of the grid, neighborhood strategies,
and settings. Implementations may involve multiple processors or graphical
processing units, with information exchange possible through physical
communication networks. The theory of parallel genetic algorithms
has been thoroughly presented by a number of researchers in the relevant
literature \citep{pga_survey1,pga_survey2}. Also, parallel genetic
algorithms have been incorporated in combinatorial optimization \citep{pga_comb}.

The proposed method is based on the island technique and suggests
a number of improvements to the general scheme of parallel Genetic
Algorithms. Among these improvements are a series of techniques for
propagating optimal solutions among islands that aim to speed up the
convergence of the overall algorithm. In addition, on the individual
islands of the genetic algorithm, a local minimization technique is
periodically applied that has two goals: to discover with the maximum
possible accuracy local minima of the objective function but also
to speed up the convergence of the overall algorithm, without wasting
computing power on values of function that have already been discovered.
Furthermore, an efficient termination rule, which is based on asymptotic
considerations and it was validated in a series of global optimization
methods is also incorporated in the current algorithm. The proposed
method was applied to a series of problems appearing in the relevant
literature. From the experimental results, it was found that the new
method is able to effectively find the global minimum of the functions
in a large percentage and moreover the above modifications significantly
accelerated the finding of the global minimum as the number of individual
...
\subsection{The Genetic Algorithm}\label{sub:genetic}
...
each working concurrently to optimize a specific set of solutions. In this figure each island implements a separate genetic algorithm as described in subsection \ref{sub:genetic}. The steps of the overall algorithm are also preseneted through a series of steps in Algorithm \ref{alg:mainAlg}.
...
\end{enumerate}
\end{enumerate}
\caption{The overall algorithm\label{alg:mainAlg}}
\end{algorithm}
...
exist on the island that sends them.The number of solutions participating
in the migration and replacement process is fully customizable and
...
To evaluate the performance of the parallel genetic algorithm, a series
of experiments were carried out. These experiments varied the number
of parallel computing units from 1 to 10. The parallelization was
achieved using the freely available OpenMP library \citep{opemp},
and the method was implemented in ANSI C++ within the OPTIMUS optimization
package, accessible at https://github.com/itsoulos/OPTIMUS. All experiments
were conducted on a system equipped with an AMD Ryzen 5950X processor,
128GB of RAM, and running the Debian Linux operating system.
...
During the conducted experiments, the \textquotedbl 1-to-N\textquotedbl{}
and \textquotedbl N-to-N\textquotedbl{} propagation techniques appear
to perform better according to experimental evidence. The common feature
of these two techniques is that the optimal solutions are distributed
to all computing units, thereby improving the performance of each
individual unit and consequently enhancing the overall performance
...
constant. Also, the parallel implementation of the GALIB library \citep{galib}
was used in the comparative experiments. The population size for all
methods is 500 particles or agents or chromosomes. In PGA, the population
consists of 20PUx25chrom, while all other parameters remain the same
as those described in Table \ref{tab:Comparison1}. Any method employing
LSR maintains this parameter at the same value. The double box is
a termination rule that is the same for all methods. 

The values resulting from experiments in the \ref{tab:Comparison4}
table are depicted in \ref{fig:Statistical4} and fig:\ref{fig:Comparison5}
Figures. The box plots of Figure \ref{fig:Statistical4} reveal the
superiority of PGA, as objective function calls remain at approximately
10,000 across all problems. Conversely, IPSO, DE, and TDE (especially
DE) exhibit a low number of calls in some problems, while in others,
they display significant increases. During initialization and optimization,
each method has a specific lower limit of calls, which varies from
method to method. PGA easily reaches this threshold with very small
deviations, as illustrated in the same figure. The Figure \ref{fig:Comparison5}
presents the total call values for each method. The current work was
also compared against the parallel version of Galib, found in the
recent literature. Even though Galib seems to have the same success
rate in discovering the global minimum of the benchmark functions,
it requires significantly more functions calls than the proposed method
for the same setup parameters.
...
\begin{longtblr}
	[
  caption = {Statistical analysis comparing execution times (seconds) and function
calls across varying numbers of processor units},
...
\textbf{\tiny{}﻿Problems} & \textbf{\tiny{}$N_{i}=1$ $N_{c}=500$ Calls} & \textbf{\tiny{}$N_{i}=1$ $N_{c}=500$ Time} & \textbf{\tiny{}$N_{i}=2$ $N_{c}=250$ Calls} & \textbf{\tiny{}$N_{i}=2$ $N_{c}=250$ Time} & \textbf{\tiny{}$N_{i}=5$ $N_{c}=100$ Calls} & \textbf{\tiny{}$N_{i}=5$ $N_{c}=100$ Time} & \textbf{\tiny{}$N_{i}=10$ $N_{c}=50$ Calls} & \textbf{\tiny{}$N_{i}=10$ $N_{c}=50$ Time}\\
\textbf{\textcolor{blue}{\tiny{}BF1}} & \textbf{\tiny{}10578} & \textbf{\tiny{}0.557} & \textbf{\tiny{}10555} & \textbf{\tiny{}0.193} & \textbf{\tiny{}10533} & \textbf{\tiny{}0.126} & \textbf{\tiny{}10511} & \textbf{\tiny{}0.121}\\
\textbf{\textcolor{blue}{\tiny{}BF2}} & \textbf{\tiny{}10568} & \textbf{\tiny{}0.554} & \textbf{\tiny{}10545} & \textbf{\tiny{}0.192} & \textbf{\tiny{}10523} & \textbf{\tiny{}0.127} & \textbf{\tiny{}10533} & \textbf{\tiny{}0.119}\\
\textbf{\textcolor{blue}{\tiny{}BRANIN}} & \textbf{\tiny{}46793} & \textbf{\tiny{}2.308} & \textbf{\tiny{}31231} & \textbf{\tiny{}0.562} & \textbf{\tiny{}11125} & \textbf{\tiny{}0.134} & \textbf{\tiny{}10533} & \textbf{\tiny{}0.169}\\
\textbf{\textcolor{blue}{\tiny{}CAMEL}} & \textbf{\tiny{}26537} & \textbf{\tiny{}1.338} & \textbf{\tiny{}15875} & \textbf{\tiny{}0.29} & \textbf{\tiny{}15833} & \textbf{\tiny{}0.188} & \textbf{\tiny{}10861} & \textbf{\tiny{}0.123}\\
\textbf{\textcolor{blue}{\tiny{}CIGAR10}} & \textbf{\tiny{}10502} & \textbf{\tiny{}1.089} & \textbf{\tiny{}10577} & \textbf{\tiny{}0.383} & \textbf{\tiny{}10583} & \textbf{\tiny{}0.222} & \textbf{\tiny{}10541} & \textbf{\tiny{}0.206}\\
\textbf{\textcolor{blue}{\tiny{}CM4}} & \textbf{\tiny{}10614} & \textbf{\tiny{}1.054} & \textbf{\tiny{}10583} & \textbf{\tiny{}0.249} & \textbf{\tiny{}10581} & \textbf{\tiny{}0.151} & \textbf{\tiny{}10556} & \textbf{\tiny{}0.139}\\
\textbf{\textcolor{blue}{\tiny{}DISCUS10}} & \textbf{\tiny{}10548} & \textbf{\tiny{}1.09} & \textbf{\tiny{}10532} & \textbf{\tiny{}0.382} & \textbf{\tiny{}10500} & \textbf{\tiny{}0.222} & \textbf{\tiny{}10502} & \textbf{\tiny{}0.205}\\
\textbf{\textcolor{blue}{\tiny{}EASOM}} & \textbf{\tiny{}100762} & \textbf{\tiny{}4.504} & \textbf{\tiny{}100610} & \textbf{\tiny{}1.66} & \textbf{\tiny{}94541} & \textbf{\tiny{}1.089} & \textbf{\tiny{}22845} & \textbf{\tiny{}0.248}\\
\textbf{\textcolor{blue}{\tiny{}ELP10}} & \textbf{\tiny{}10601} & \textbf{\tiny{}1.15} & \textbf{\tiny{}10590} & \textbf{\tiny{}0.436} & \textbf{\tiny{}10574} & \textbf{\tiny{}0.26} & \textbf{\tiny{}10557} & \textbf{\tiny{}0.242}\\
\textbf{\textcolor{blue}{\tiny{}EXP4}} & \textbf{\tiny{}16621} & \textbf{\tiny{}1.092} & \textbf{\tiny{}10587} & \textbf{\tiny{}0.249} & \textbf{\tiny{}10560} & \textbf{\tiny{}0.15} & \textbf{\tiny{}10544} & \textbf{\tiny{}0.143}\\
\textbf{\textcolor{blue}{\tiny{}EXP16}} & \textbf{\tiny{}10680} & \textbf{\tiny{}1.336} & \textbf{\tiny{}10654} & \textbf{\tiny{}0.53} & \textbf{\tiny{}10643} & \textbf{\tiny{}0.287} & \textbf{\tiny{}10626} & \textbf{\tiny{}0.258}\\
\textbf{\textcolor{blue}{\tiny{}EXP64}} & \textbf{\tiny{}10857} & \textbf{\tiny{}2.333} & \textbf{\tiny{}10829} & \textbf{\tiny{}1.235} & \textbf{\tiny{}10814} & \textbf{\tiny{}0.825} & \textbf{\tiny{}10830} & \textbf{\tiny{}0.728}\\
\textbf{\textcolor{blue}{\tiny{}EXP100}} & \textbf{\tiny{}10855} & \textbf{\tiny{}3.517} & \textbf{\tiny{}10901} & \textbf{\tiny{}1.763} & \textbf{\tiny{}10868} & \textbf{\tiny{}1.25} & \textbf{\tiny{}10887} & \textbf{\tiny{}1.052}\\
\textbf{\textcolor{blue}{\tiny{}GKLS250}} & \textbf{\tiny{}50804} & \textbf{\tiny{}2.825} & \textbf{\tiny{}25832} & \textbf{\tiny{}0.607} & \textbf{\tiny{}11711} & \textbf{\tiny{}0.194} & \textbf{\tiny{}10870(93)} & \textbf{\tiny{}0.198}\\
\textbf{\textcolor{blue}{\tiny{}GKLS350}} & \textbf{\tiny{}40707} & \textbf{\tiny{}2.327} & \textbf{\tiny{}23720} & \textbf{\tiny{}0.522} & \textbf{\tiny{}17646} & \textbf{\tiny{}0.26} & \textbf{\tiny{}14130} & \textbf{\tiny{}0.202}\\
\textbf{\textcolor{blue}{\tiny{}GRIEWANK2}} & \textbf{\tiny{}10555} & \textbf{\tiny{}0.565} & \textbf{\tiny{}10532} & \textbf{\tiny{}0.197} & \textbf{\tiny{}10517} & \textbf{\tiny{}0.126} & \textbf{\tiny{}10492} & \textbf{\tiny{}0.118}\\
\textbf{\textcolor{blue}{\tiny{}GRIEWANK10}} & \textbf{\tiny{}10679} & \textbf{\tiny{}1.079} & \textbf{\tiny{}10629} & \textbf{\tiny{}0.407} & \textbf{\tiny{}10613} & \textbf{\tiny{}0.239} & \textbf{\tiny{}10609} & \textbf{\tiny{}0.22}\\
\textbf{\textcolor{blue}{\tiny{}POTENTIAL3}} & \textbf{\tiny{}39607} & \textbf{\tiny{}2.057} & \textbf{\tiny{}34327} & \textbf{\tiny{}0.881} & \textbf{\tiny{}18313} & \textbf{\tiny{}0.34} & \textbf{\tiny{}15471} & \textbf{\tiny{}0.279}\\
\textbf{\textcolor{blue}{\tiny{}PONTENTIAL5}} & \textbf{\tiny{}33542} & \textbf{\tiny{}1.653} & \textbf{\tiny{}33737} & \textbf{\tiny{}1.074} & \textbf{\tiny{}12040} & \textbf{\tiny{}0.34} & \textbf{\tiny{}11082} & \textbf{\tiny{}0.291}\\
\textbf{\textcolor{blue}{\tiny{}PONTENTIAL6}} & \textbf{\tiny{}28901(3)} & \textbf{\tiny{}1.56} & \textbf{\tiny{}26419(16)} & \textbf{\tiny{}1.018} & \textbf{\tiny{}14265(3)} & \textbf{\tiny{}0.478} & \textbf{\tiny{}11109(10)} & \textbf{\tiny{}0.356}\\
\textbf{\textcolor{blue}{\tiny{}PONTENTIAL10}} & \textbf{\tiny{}42644(13)} & \textbf{\tiny{}3.316} & \textbf{\tiny{}37897(23)} & \textbf{\tiny{}2.538} & \textbf{\tiny{}14080(10)} & \textbf{\tiny{}0.937} & \textbf{\tiny{}11319(6)} & \textbf{\tiny{}0.66}\\
\textbf{\textcolor{blue}{\tiny{}HANSEN}} & \textbf{\tiny{}46894(90)} & \textbf{\tiny{}2.494} & \textbf{\tiny{}28191(80)} & \textbf{\tiny{}0.575} & \textbf{\tiny{}11085(56)} & \textbf{\tiny{}0.153} & \textbf{\tiny{}11065} & \textbf{\tiny{}0.158}\\
\textbf{\textcolor{blue}{\tiny{}HARTMAN3}} & \textbf{\tiny{}22235} & \textbf{\tiny{}1.525} & \textbf{\tiny{}19030} & \textbf{\tiny{}0.379} & \textbf{\tiny{}16463} & \textbf{\tiny{}0.212} & \textbf{\tiny{}12048} & \textbf{\tiny{}0.146}\\
\textbf{\textcolor{blue}{\tiny{}HARTMAN6}} & \textbf{\tiny{}18352} & \textbf{\tiny{}1.505} & \textbf{\tiny{}15902} & \textbf{\tiny{}0.429} & \textbf{\tiny{}16726} & \textbf{\tiny{}0.279} & \textbf{\tiny{}12243} & \textbf{\tiny{}0.196}\\
\textbf{\textcolor{blue}{\tiny{}RASTRIGIN}} & \textbf{\tiny{}16567} & \textbf{\tiny{}0.855} & \textbf{\tiny{}10543} & \textbf{\tiny{}0.193} & \textbf{\tiny{}10521} & \textbf{\tiny{}0.125} & \textbf{\tiny{}10506} & \textbf{\tiny{}0.116}\\
\textbf{\textcolor{blue}{\tiny{}ROSENBROCK8}} & \textbf{\tiny{}10863} & \textbf{\tiny{}0.916} & \textbf{\tiny{}10700} & \textbf{\tiny{}0.333} & \textbf{\tiny{}10698} & \textbf{\tiny{}0.199} & \textbf{\tiny{}10772} & \textbf{\tiny{}0.196}\\
\textbf{\textcolor{blue}{\tiny{}POSENBROCK16}} & \textbf{\tiny{}10918} & \textbf{\tiny{}1.371} & \textbf{\tiny{}10946} & \textbf{\tiny{}0.516} & \textbf{\tiny{}10867} & \textbf{\tiny{}0.304} & \textbf{\tiny{}10886} & \textbf{\tiny{}0.271}\\
\textbf{\textcolor{blue}{\tiny{}SHEKEL5}} & \textbf{\tiny{}32319(50)} & \textbf{\tiny{}2.069} & \textbf{\tiny{}17913(50)} & \textbf{\tiny{}0.412} & \textbf{\tiny{}11185(36)} & \textbf{\tiny{}0.159} & \textbf{\tiny{}11010(40)} & \textbf{\tiny{}0.15}\\
\textbf{\textcolor{blue}{\tiny{}SHEKEL7}} & \textbf{\tiny{}51183(73)} & \textbf{\tiny{}3.277} & \textbf{\tiny{}14981(53)} & \textbf{\tiny{}0.342} & \textbf{\tiny{}11457(60)} & \textbf{\tiny{}0.163} & \textbf{\tiny{}11035(50)} & \textbf{\tiny{}0.154}\\
\textbf{\textcolor{blue}{\tiny{}SHEKEL10}} & \textbf{\tiny{}47337(70)} & \textbf{\tiny{}2.977} & \textbf{\tiny{}46927(76)} & \textbf{\tiny{}1.113} & \textbf{\tiny{}16310(56)} & \textbf{\tiny{}0.23} & \textbf{\tiny{}11329(70)} & \textbf{\tiny{}0.152}\\
\textbf{\textcolor{blue}{\tiny{}SINU4}} & \textbf{\tiny{}66625(83)} & \textbf{\tiny{}4.344} & \textbf{\tiny{}31511(86)} & \textbf{\tiny{}0.77} & \textbf{\tiny{}13979(73)} & \textbf{\tiny{}0.211} & \textbf{\tiny{}11004(43)} & \textbf{\tiny{}0.161}\\
\textbf{\textcolor{blue}{\tiny{}SINU8}} & \textbf{\tiny{}29705} & \textbf{\tiny{}2.57} & \textbf{\tiny{}27613} & \textbf{\tiny{}0.987} & \textbf{\tiny{}24592} & \textbf{\tiny{}0.549} & \textbf{\tiny{}11422} & \textbf{\tiny{}0.236}\\
\textbf{\textcolor{blue}{\tiny{}TEST2N4}} & \textbf{\tiny{}25553} & \textbf{\tiny{}1.558} & \textbf{\tiny{}17701} & \textbf{\tiny{}0.397} & \textbf{\tiny{}24763} & \textbf{\tiny{}0.359} & \textbf{\tiny{}13217} & \textbf{\tiny{}0.178}\\
\textbf{\textcolor{blue}{\tiny{}TEST2N5}} & \textbf{\tiny{}20297} & \textbf{\tiny{}1.327} & \textbf{\tiny{}18440} & \textbf{\tiny{}0.457} & \textbf{\tiny{}16759} & \textbf{\tiny{}0.265} & \textbf{\tiny{}11483} & \textbf{\tiny{}0.168}\\
\textbf{\textcolor{blue}{\tiny{}TEST2N6}} & \textbf{\tiny{}20450} & \textbf{\tiny{}1.311} & \textbf{\tiny{}20837} & \textbf{\tiny{}0.566} & \textbf{\tiny{}18123} & \textbf{\tiny{}0.315} & \textbf{\tiny{}11988} & \textbf{\tiny{}0.194}\\
\textbf{\textcolor{blue}{\tiny{}TEST2N7}} & \textbf{\tiny{}26113} & \textbf{\tiny{}1.924} & \textbf{\tiny{}23940} & \textbf{\tiny{}0.723} & \textbf{\tiny{}20825} & \textbf{\tiny{}0.384} & \textbf{\tiny{}11339} & \textbf{\tiny{}0.196}\\
\textbf{\textcolor{blue}{\tiny{}TEST2N8}} & \textbf{\tiny{}18846} & \textbf{\tiny{}1.454} & \textbf{\tiny{}18549} & \textbf{\tiny{}0.585} & \textbf{\tiny{}16700} & \textbf{\tiny{}0.329} & \textbf{\tiny{}11658} & \textbf{\tiny{}0.218}\\
\textbf{\textcolor{blue}{\tiny{}TEST2N9}} & \textbf{\tiny{}18154} & \textbf{\tiny{}1.582} & \textbf{\tiny{}18803} & \textbf{\tiny{}0.649} & \textbf{\tiny{}17100} & \textbf{\tiny{}0.368} & \textbf{\tiny{}13299} & \textbf{\tiny{}0.262}\\
\textbf{\textcolor{blue}{\tiny{}TEST30N3}} & \textbf{\tiny{}49235} & \textbf{\tiny{}2.46} & \textbf{\tiny{}24129} & \textbf{\tiny{}0.458} & \textbf{\tiny{}14743} & \textbf{\tiny{}0.188} & \textbf{\tiny{}12345} & \textbf{\tiny{}0.152}\\
\textbf{\textcolor{blue}{\tiny{}TEST30N4}} & \textbf{\tiny{}29667} & \textbf{\tiny{}1.553} & \textbf{\tiny{}17501} & \textbf{\tiny{}0.358} & \textbf{\tiny{}13367} & \textbf{\tiny{}0.186} & \textbf{\tiny{}11778} & \textbf{\tiny{}0.151}\\
\textbf{\textcolor{green}{\tiny{}SUM}} & \textbf{\textcolor{green}{\tiny{}1105268}} & \textbf{\textcolor{green}{\tiny{}74.376}} & \textbf{\textcolor{green}{\tiny{}851319}} & \textbf{\textcolor{green}{\tiny{}25.61}} & \textbf{\textcolor{green}{\tiny{}633126}} & \textbf{\textcolor{green}{\tiny{}12.923}} & \textbf{\textcolor{green}{\tiny{}465835}} & \textbf{\textcolor{green}{\tiny{}9.532}}\\
\textbf{\textcolor{green}{\tiny{}MINIMUM}} & \textbf{\textcolor{green}{\tiny{}10502 }} & \textbf{\textcolor{green}{\tiny{}0.554 }} & \textbf{\textcolor{green}{\tiny{}10532 }} & \textbf{\textcolor{green}{\tiny{}0.192 }} & \textbf{\textcolor{green}{\tiny{}10500 }} & \textbf{\textcolor{green}{\tiny{}0.125 }} & \textbf{\textcolor{green}{\tiny{}10492 }} & \textbf{\textcolor{green}{\tiny{}0.116 }}\\
\textbf{\textcolor{green}{\tiny{}MAXIMUM}} & \textbf{\textcolor{green}{\tiny{}100762 }} & \textbf{\textcolor{green}{\tiny{}4.504 }} & \textbf{\textcolor{green}{\tiny{}100610 }} & \textbf{\textcolor{green}{\tiny{}2.538 }} & \textbf{\textcolor{green}{\tiny{}94541 }} & \textbf{\textcolor{green}{\tiny{}1.25 }} & \textbf{\textcolor{green}{\tiny{}22845 }} & \textbf{\textcolor{green}{\tiny{}1.052 }}\\
\textbf{\textcolor{green}{\tiny{}AVERAGE}} & \textbf{\textcolor{green}{\tiny{}27631.7 }} & \textbf{\textcolor{green}{\tiny{}1.859 }} & \textbf{\textcolor{green}{\tiny{}21282.975 }} & \textbf{\textcolor{green}{\tiny{}0.640 }} & \textbf{\textcolor{green}{\tiny{}15828.15 }} & \textbf{\textcolor{green}{\tiny{}0.323 }} & \textbf{\textcolor{green}{\tiny{}11645.875 }} & \textbf{\textcolor{green}{\tiny{}0.238 }}\\
\textbf{\textcolor{green}{\tiny{}STDEV}} & \textbf{\textcolor{green}{\tiny{}19305.784 }} & \textbf{\textcolor{green}{\tiny{}0.972 }} & \textbf{\textcolor{green}{\tiny{}15829.020 }} & \textbf{\textcolor{green}{\tiny{}0.482 }} & \textbf{\textcolor{green}{\tiny{}13335.509 }} & \textbf{\textcolor{green}{\tiny{}0.260 }} & \textbf{\textcolor{green}{\tiny{}2109.230 }} & \textbf{\textcolor{green}{\tiny{}0.180 }}
...
\hline 
\textbf{\scriptsize{}﻿PROBLEMS} & \textbf{\scriptsize{}PSO} & \textbf{\scriptsize{}IPSO} & \textbf{\scriptsize{}RDE} & \textbf{\scriptsize{}TDE} & \textbf{\scriptsize{}GA} & \textbf{\scriptsize{}GALIB} & \textbf{\scriptsize{}PGA}\tabularnewline
\hline 
\hline 
{\scriptsize{}BF1} & {\scriptsize{}50398} & {\scriptsize{}11478} & {\scriptsize{}7943(86)} & {\scriptsize{}5535} & {\scriptsize{}10578} & {\scriptsize{}11641} & {\scriptsize{}10501}\tabularnewline
\hline 
{\scriptsize{}BF2} & {\scriptsize{}50397} & {\scriptsize{}11292} & {\scriptsize{}8472(76)} & {\scriptsize{}5539} & {\scriptsize{}10568} & {\scriptsize{}11321} & {\scriptsize{}10510}\tabularnewline
\hline 
{\scriptsize{}BRANIN} & {\scriptsize{}44800} & {\scriptsize{}10849} & {\scriptsize{}5513} & {\scriptsize{}5514} & {\scriptsize{}46793} & {\scriptsize{}34487} & {\scriptsize{}10838}\tabularnewline
\hline 
{\scriptsize{}CAMEL} & {\scriptsize{}48242} & {\scriptsize{}11051} & {\scriptsize{}5555} & {\scriptsize{}5514} & {\scriptsize{}26537} & {\scriptsize{}17321} & {\scriptsize{}11087}\tabularnewline
\hline 
{\scriptsize{}CIGAR10} & {\scriptsize{}50581} & {\scriptsize{}12331} & {\scriptsize{}5586} & {\scriptsize{}100573} & {\scriptsize{}10502} & {\scriptsize{}11567(50)} & {\scriptsize{}10566}\tabularnewline
\hline 
{\scriptsize{}CM4} & {\scriptsize{}48559} & {\scriptsize{}11767} & {\scriptsize{}5550} & {\scriptsize{}5538} & {\scriptsize{}10614} & {\scriptsize{}11118(70)} & {\scriptsize{}10548}\tabularnewline
\hline 
{\scriptsize{}DISCUS10} & {\scriptsize{}50523} & {\scriptsize{}14328} & {\scriptsize{}18187} & {\scriptsize{}100518} & {\scriptsize{}10548} & {\scriptsize{}10988} & {\scriptsize{}10503}\tabularnewline
\hline 
{\scriptsize{}EASOM} & {\scriptsize{}21786} & {\scriptsize{}10938} & {\scriptsize{}29256} & {\scriptsize{}24691} & {\scriptsize{}100762} & {\scriptsize{}79689} & {\scriptsize{}10797}\tabularnewline
\hline 
{\scriptsize{}ELP10} & {\scriptsize{}49837} & {\scriptsize{}4323} & {\scriptsize{}11933} & {\scriptsize{}100584} & {\scriptsize{}10601} & {\scriptsize{}11673} & {\scriptsize{}10559}\tabularnewline
\hline 
{\scriptsize{}EXP4} & {\scriptsize{}48523} & {\scriptsize{}11041} & {\scriptsize{}46752} & {\scriptsize{}19467} & {\scriptsize{}16621} & {\scriptsize{}16045} & {\scriptsize{}10503}\tabularnewline
\hline 
{\scriptsize{}EXP16} & {\scriptsize{}50518} & {\scriptsize{}10973} & {\scriptsize{}5537} & {\scriptsize{}69494} & {\scriptsize{}10680} & {\scriptsize{}10500} & {\scriptsize{}10595}\tabularnewline
\hline 
{\scriptsize{}GKLS250} & {\scriptsize{}43925} & {\scriptsize{}10869} & {\scriptsize{}41016} & {\scriptsize{}11430} & {\scriptsize{}50804} & {\scriptsize{}31298} & {\scriptsize{}10893(76)}\tabularnewline
\hline 
{\scriptsize{}GKLS350} & {\scriptsize{}48202} & {\scriptsize{}10750} & {\scriptsize{}56220} & {\scriptsize{}16831} & {\scriptsize{}40707} & {\scriptsize{}29897(96)} & {\scriptsize{}11555(96)}\tabularnewline
\hline 
{\scriptsize{}GRIEWANK2} & {\scriptsize{}44021} & {\scriptsize{}13514} & {\scriptsize{}5538} & {\scriptsize{}5533} & {\scriptsize{}10555} & {\scriptsize{}14419(67)} & {\scriptsize{}10498}\tabularnewline
\hline 
{\scriptsize{}GRIEWANK10} & {\scriptsize{}50557(3)} & {\scriptsize{}12258(86)} & {\scriptsize{}5612(13)} & {\scriptsize{}85742(3)} & {\scriptsize{}10679} & {\scriptsize{}10800} & {\scriptsize{}10576}\tabularnewline
\hline 
{\scriptsize{}POTENTIAL3} & {\scriptsize{}49213} & {\scriptsize{}12124} & {\scriptsize{}5530} & {\scriptsize{}5523} & {\scriptsize{}39607} & {\scriptsize{}33452} & {\scriptsize{}11039}\tabularnewline
\hline 
{\scriptsize{}PONTENTIAL5} & {\scriptsize{}50548} & {\scriptsize{}16027} & {\scriptsize{}5587} & {\scriptsize{}5569} & {\scriptsize{}33542} & {\scriptsize{}31285} & {\scriptsize{}11134}\tabularnewline
\hline 
{\scriptsize{}PONTENTIAL6} & {\scriptsize{}50558(3)} & {\scriptsize{}24414(66)} & {\scriptsize{}5607(6)} & {\scriptsize{}5588(3)} & {\scriptsize{}28901(3)} & {\scriptsize{}28444(10)} & {\scriptsize{}11143(10)}\tabularnewline
\hline 
{\scriptsize{}PONTENTIAL10} & {\scriptsize{}50641(6)} & {\scriptsize{}31434} & {\scriptsize{}5670(3)} & {\scriptsize{}5661(6)} & {\scriptsize{}42644(13)} & {\scriptsize{}38883(20)} & {\scriptsize{}11290(20)}\tabularnewline
\hline 
{\scriptsize{}HANSEN} & {\scriptsize{}47296} & {\scriptsize{}13131} & {\scriptsize{}5522} & {\scriptsize{}5521} & {\scriptsize{}46894(90)} & {\scriptsize{}45440} & {\scriptsize{}11055}\tabularnewline
\hline 
{\scriptsize{}HARTMAN3} & {\scriptsize{}47778} & {\scriptsize{}10961} & {\scriptsize{}5525} & {\scriptsize{}5522} & {\scriptsize{}22235} & {\scriptsize{}19434} & {\scriptsize{}11097}\tabularnewline
\hline 
{\scriptsize{}HARTMAN6} & {\scriptsize{}50088(33)} & {\scriptsize{}11085(86)} & {\scriptsize{}5536(83)} & {\scriptsize{}5536} & {\scriptsize{}18352} & {\scriptsize{}18444(60)} & {\scriptsize{}11273}\tabularnewline
\hline 
{\scriptsize{}RASTRIGIN} & {\scriptsize{}47433} & {\scriptsize{}11594} & {\scriptsize{}5542} & {\scriptsize{}5524} & {\scriptsize{}16567} & {\scriptsize{}16286(96)} & {\scriptsize{}10506}\tabularnewline
\hline 
{\scriptsize{}ROSENBROCK8} & {\scriptsize{}50549} & {\scriptsize{}13487} & {\scriptsize{}72088} & {\scriptsize{}100503} & {\scriptsize{}10863} & {\scriptsize{}11419} & {\scriptsize{}10645}\tabularnewline
\hline 
{\scriptsize{}POSENBROCK16} & {\scriptsize{}50584} & {\scriptsize{}12659} & {\scriptsize{}21517} & {\scriptsize{}10645} & {\scriptsize{}10918} & {\scriptsize{}11681} & {\scriptsize{}10957}\tabularnewline
\hline 
{\scriptsize{}SHEKEL5} & {\scriptsize{}49944(33)} & {\scriptsize{}13058(93)} & {\scriptsize{}5532(86)} & {\scriptsize{}5524(93)} & {\scriptsize{}32319(50)} & {\scriptsize{}29287} & {\scriptsize{}10883(43)}\tabularnewline
\hline 
{\scriptsize{}SHEKEL7} & {\scriptsize{}50062(53)} & {\scriptsize{}12134(96)} & {\scriptsize{}5533(96)} & {\scriptsize{}5523} & {\scriptsize{}51183(73)} & {\scriptsize{}47245(77)} & {\scriptsize{}10926(53)}\tabularnewline
\hline 
{\scriptsize{}SHEKEL10} & {\scriptsize{}50124(63)} & {\scriptsize{}14176} & {\scriptsize{}5535(90)} & {\scriptsize{}5523} & {\scriptsize{}47337(70)} & {\scriptsize{}45911(77)} & {\scriptsize{}11207(80)}\tabularnewline
\hline 
{\scriptsize{}SINU4} & {\scriptsize{}49239} & {\scriptsize{}11349} & {\scriptsize{}5527} & {\scriptsize{}5510} & {\scriptsize{}66625(83)} & {\scriptsize{}66383} & {\scriptsize{}11063(76)}\tabularnewline
\hline 
{\scriptsize{}SINU8} & {\scriptsize{}50224} & {\scriptsize{}11295} & {\scriptsize{}5537(80)} & {\scriptsize{}5520} & {\scriptsize{}29705} & {\scriptsize{}29234} & {\scriptsize{}11378}\tabularnewline
\hline 
{\scriptsize{}TEST2N4} & {\scriptsize{}50112(93)} & {\scriptsize{}13173} & {\scriptsize{}5529} & {\scriptsize{}5519} & {\scriptsize{}25553} & {\scriptsize{}19913} & {\scriptsize{}11049}\tabularnewline
\hline 
{\scriptsize{}TEST2N9} & {\scriptsize{}50517(13)} & {\scriptsize{}17510(60)} & {\scriptsize{}5546(6)} & {\scriptsize{}5535(56)} & {\scriptsize{}18154} & {\scriptsize{}15376} & {\scriptsize{}11145}\tabularnewline
\hline 
{\scriptsize{}TEST30N3} & {\scriptsize{}44301} & {\scriptsize{}19638} & {\scriptsize{}5515} & {\scriptsize{}5511} & {\scriptsize{}49235} & {\scriptsize{}49234} & {\scriptsize{}11051}\tabularnewline
\hline 
{\scriptsize{}TEST30N4} & {\scriptsize{}49177} & {\scriptsize{}20839} & {\scriptsize{}5514} & {\scriptsize{}5511} & {\scriptsize{}29667} & {\scriptsize{}33428} & {\scriptsize{}11301}\tabularnewline
\hline 
\textbf{\scriptsize{}TOTAL} & \textbf{\footnotesize{}1639257} & \textbf{\footnotesize{}457850} & \textbf{\footnotesize{}446562} & \textbf{\footnotesize{}767771} & \textbf{\footnotesize{}997850} & \textbf{\scriptsize{}903547} & \textbf{\footnotesize{}370671}\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
\end{table}
...
accelerated. Additionally, a new experiment was conducted where the
number of chromosomes was varied from 250 to 1000 and the number of
processing units was changed from 1 to 10. The total number of function
calls for each case is shown graphically in Figure \ref{fig:compareChromosomes}.
The method maintains the same behavior for any number of chromosomes.
This means that the set of required calls is significantly reduced
by adding new parallel processing units. Of course, as expected, the
total number of calls required increases as the number of available
chromosomes increases.

\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.5]{newimg}
\par\end{centering}
\caption{Comparison of function calls with different number of chromosomes.\label{fig:compareChromosomes}}
...
\bibitem{pga_comb}H. Mühlenbein, Parallel genetic algorithms in combinatorial
optimization. In Computer science and operations research, pp. 441-453,
Pergamon, 1992.
...
\bibitem{galib}M. Wall, GAlib: A C++ library of genetic algorithm
components. Mechanical Engineering Department, Massachusetts Institute
of Technology. 1996 Aug;87:54.
