...
are proposed in the direction of the efficient parallelization for
the Particle Swarm Optimization. These modifications include an innovative
velocity calculation mechanism that has also been successfully used
in the serial version of the method, mechanisms for propagating the
best particles between parallel computing units, but also a process
termination mechanism, which has been properly configured for efficient
execution on parallel computing environments. The proposed technique
was applied to a multitude of computational problems from the relevant
literature and the results were more than promising, since it was
found that increasing the computational threads can significantly
reduce the required number of function calls to find the global minimum.}
...
\]
where the function f is assumed to be continuous and differentiable.There
is a wide range of problems in the relevant literature that can be
treated as global optimization problems such as problems in physical
science \citep{go_physics1,go_physics2,go_physics3}, chemistry \citep{go_chem1,go_chem2,go_chem3},
economics \citep{go_econ1,go_econ2} and medicine \citep{go_med1,go_med2}.
...
objective function. On the other hand, in the category of stochastic
techniques, where the greatest research effort is made, techniques
are presented that look for the total minimum using stochastic methods,
which of course does not guarantee finding the total minimum. In this
vast area of research endeavors one encounters methods such as controlled
random search methods\citep{crs1,crs2,crs3} which can be considered
as a direct search method, simulated annealing methods \citep{simann_major,simann1,simann2},
...
of each particle and the velocity with which it moves in search of
the global minimum. The current position is denoted as $\overrightarrow{x}$
...
problems from all areas of the sciences such as problems that appear
...
and Walczak \citep{psoReview}. Also a recent overview of PSO variants
can be found in the work of Jain et al \citep{psoOverview}. Since
the technique has gained a lot of popularity in recent years, many
works have been developed based on it, such as combination with the
mutation mechanism \citep{pso_mutation1,pso_mutation2,pso_mutation3},
...
Also since the calculation of the velocity of the particles is a determining
factor for the effectiveness of the method, several techniques were
formulated for the calculation of the velocity \citep{pso_velocity1,pso_velocity2,pso_velocity3}.
...
that run independently on parallel computing units. The same serial
algorithm with the same set of parameters is applied to each computing
unit. Also, a series of modifications to the original particle swarm
optimization method are proposed in order to make it more efficient
in parallel computing environments. These modifications include a
new method of calculating particle velocity, a new termination rule
...
by Eberhart and Shi \citep{RandomInertia} and it is based in random
...
a series of well - known optimization problems. Although, the proposed
...
\subsection{The main steps of the basic PSO algorithm\label{subsec:The-base-algorithm}}
...
proposed method was also evaluated.

\subsection{Test functions}

The mathematical formulas for the series of objective functions used
are presented below: given below
\begin{itemize}
\item \textbf{\emph{Bent Cigar function}}\emph{ }The function is 
\[
f(x)=x_{1}^{2}+10^{6}\sum_{i=2}^{n}x_{i}^{2}
\]
For the conducted experiments the value $n=10$ was used as test case
and the function is denoted as CIGAR10 in the experimental tables.
...
was used and the corresponding name for the experimental tables is
CM4.
...
\]
For the conducted experiments the value $n=10$ was used and the name
used in the experimental tables is DISCUS10.
...
\]
 In the conducted experiments the values $n=4,16,64$ were used and
the associated names in the experimental tables are EXP4, EXP16 and
EXP64 correspondingly.
\item \textbf{Griewank2} function:
\[
f(x)=1+\frac{1}{200}\sum_{i=1}^{2}x_{i}^{2}-\prod_{i=1}^{2}\frac{\cos(x_{i})}{\sqrt{(i)}}
\]
where $x\in[-100,100]^{2}$.
\item \textbf{Gkls} function. $f(x)=\mbox{Gkls}(x,n,w)$. This function
has been proposed in \citep{gkls} and it is constructed in a way
that it has $w$ local minima in the range $[-1,1]^{n}$. The value
of $n$ is between 2 and 100. The global minimum for this function
is -1 and in the conducted experiments we have used the values $n=2,3$
and $w=50,\ 100$. The corresponding function names in the experimental
tables are GKLS250 and GKLS350 respectively.
...
N atoms interacting with each other using the Lennard - Jones potential
\citep{Jones}. This potential is defined as
\begin{equation}
V_{LJ}(r)=4\epsilon\left[\left(\frac{\sigma}{r}\right)^{12}-\left(\frac{\sigma}{r}\right)^{6}\right]\label{eq:potential}
\end{equation}
In the conducted experiments the values $N=3,\ 5$ were used and the
corresponding function names are POTENTIAL3 and POTENTIAL5.
...
\]
The values $n=4,\ 8$ were used in the conducted experiments and the
associated function names are ROSENBROCK4 and ROSENBROCK8 respectively.
...
\]
The values of $n=4,8$ were used in the experiments and the associated
function names are SINU4 and SINU8 respectively. The value of parameter
$z$ was set to $\frac{\pi}{6}$.
\item \textbf{Test2N} function:
\[
f(x)=\frac{1}{2}\sum_{i=1}^{n}x_{i}^{4}-16x_{i}^{2}+5x_{i},\quad x_{i}\in[-5,5].
\]
The function has $2^{n}$ and the values $n=4,5,6,7$ were used and
the associated function names are TEST2N4, TEST2N5, TEST2N6 and TEST2N7
respectively.
\item \textbf{Test30N} function:
\[
f(x)=\frac{1}{10}\sin^{2}\left(3\pi x_{1}\right)\sum_{i=2}^{n-1}\left(\left(x_{i}-1\right)^{2}\left(1+\sin^{2}\left(3\pi x_{i+1}\right)\right)\right)+\left(x_{n}-1\right)^{2}\left(1+\sin^{2}\left(2\pi x_{n}\right)\right)
\]
with $x\in[-10,10]$, with $30^{n}$ local minima and the values $n=3,4$
...
function values for 30 runs. Also, the numbers in parentheses stand
...
this experiment is shown in Figure \ref{fig:boxplot1to1}.In addition,
in order to give greater reliability to the experimental results,
the total number of particles was in all runs constant and equal to
200, as defined in the table \ref{tab:values}. This means, for example,
that when the computing units are 4 each unit will run a serial version
of PSO with 50 particles and when the computing units become 5 the
number of particles in each unit will be reduced to 40.
