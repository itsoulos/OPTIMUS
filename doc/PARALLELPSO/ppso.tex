...
\usepackage{babel}
\begin{document}
\title{An improved parallel Particle Swarm Optimization}
\author{Vasileios Charilogis, Ioannis G. Tsoulos$^{*}$, Alexandros Tzallas }
\maketitle

\lyxaddress{Department of Informatics and Telecommunications, University of Ioannina}
\begin{abstract}
In the area of global optimization, a variety of techniques have been
developed to find the global minimum. These techniques, in most cases,
require a significant amount of computational resources and time to
complete and therefore there is a need to develop parallel techniques.
In addition, the wide spread of parallel architectures in recent years
greatly facilitates the implementation of such techniques. Among the
most widely used global optimization techniques is the Particle Swarm
Optimization technique. In this work, a series of modifications are
proposed in the direction of efficient parallelization for Particle
...
\end{abstract}
\textbf{Keywords}: Optimization, Parallel methods, Evolutionary techniques,
Stochastic methods, Termination rules.
...
an accelerator lattice\cite{go_physics1}, Iuliano proposed global
...
Models\cite{go_physics3}. In the area of chemistry, Heils and Johnston
provided a review on the usage of global optimization techniques on
clusters\cite{go_chem1}, Shin et al proposed a software that utilizes
global optimization methods for protein - ligand docking\cite{go_chem2}
and Liwo et al proposed a global optimization algorithm for protein
structure prediction by minimizing the potential energy function\cite{go_chem3}.
In addition, in the area of economic sciences, Gaing proposed a Particle
Swarm Optimization method to solve the economic dispatch\cite{go_econ1}
and Maranas et al proposed global optimization methods for tackling
long - term financial planning problems\cite{go_econ2}. Furthermore,
in medicine, Lee proposed large - scale optimization - based classification
models applied in medical problems\cite{go_med1} and Cherruault reviewed
some global optimization methods that were applied to medicine problems\cite{go_med2}. 
...
the Interval method\cite{interval1,interval2}, where the initial
...
determination \cite{determ1}, the TRUST method for global optimization
\cite{determ2}, the method of Evtushenko and Posypkin for global
box - constrained optimization \cite{determ3}, a method that uses
smooth diagonal auxiliary functions to approximate the global minimum
\cite{determ4} etc. Also, Kunde et al \cite{determ5} suggested the
usage of deterministic global optimization methods for chemical problems.
Furthermore, recently, Sergeyev et al \cite{Sergeyev} proposed a
systematic comparison between deterministic and stochastic methods
used to handle the global optimization problem. Recently, Yassami
and Ashtari proposed a hybrid optimization method to handle optimization
problems \cite{dynamicMethod}. 
...
search methods\cite{crs1,crs2,crs3} which can be considered as a
direct search method, simulated annealing methods \cite{simann_major,simann1,simann2},
...
\cite{aco1,aco2}, Genetic Algorithms \cite{ga1,ga2,ga3}, etc. In
addition, since in recent years there has been an extremely wide spread
of parallel computing units, many studies have appeared that utilize
the modern parallel processing units \cite{gpu1,gpu2,gpu3} to tackle
the global optimization problem. Some research that one can study
regarding metaheuristic algorithms is presented in some recent papers
\cite{meta1,meta2,meta3}. This paper suggests a number of directions
for efficient parallelization of particle swarm optimization (PSO)
techniques.

Parallel optimization methods can be utilized in various application
domains, enabling rapid resolution of complex problems. Some of these
domains include: 
\begin{enumerate}
\item Engineering and Design: Optimizing parameters for improved performance
in engineering systems like aircraft, automobiles, and machinery. 
\item Computer Science: Optimizing complex algorithms and computational
tasks. 
\item Biology and Pharmaceuticals: Addressing problems like DNA analysis
and calculating multiple protein interactions. 
\item Economics and Finance: Automating the optimization of intricate models. 
\item Energy Optimization: Optimizing energy consumption in energy networks. 
\item Climate Change: Optimizing complex climatic models and weather prediction. 
\item Telecommunications: Designing efficient communication networks and
optimizing network performance and bandwidth management. 
\item Policy Decision-Making: Finding optimal solutions for problems like
transportation routing and sustainability policies.
\end{enumerate}
By applying parallel optimization techniques in these diverse fields,
researchers and practitioners can efficiently tackle complex challenges
and achieve optimal solutions more effectively.
...
in physics\textbf{ }\cite{psophysics1,psophysics2}, chemistry \cite{psochem1,psochem2},
medicine \cite{psomed1,psomed2}, economics \cite{psoecon} etc. Also,
the PSO method was successfully applied recently in many practical
problems such as flow shop scheduling \cite{psoApp1}, successful
development of electric vehicle charging strategies \cite{psoApp2},
emotion recognition \cite{psoApp3}, robotics \cite{psoApp4} etc.
A extensive tutorial on PSO methods can be found in the work of Marini
and Walczak \cite{psoReview}. Moreover, a recent overview of PSO
variants can be found in the work of Jain et al \cite{psoOverview}.
...
the mutation mechanism \cite{pso_mutation1,pso_mutation2,pso_mutation3},
methods that propose new techniques to initialize the velocity vector
\cite{pso_initvelocity},\textbf{ }hybrid techniques \cite{psohybrid1,psohybrid2,psohybrid3},
parallel techniques \cite{pso_parallel1,pso_parallel2,pso_parallel3}.
...
were formulated for the calculation of the velocity \cite{pso_velocity1,pso_velocity2,pso_velocity3}.
...
et al \cite{ge_pso1} who suggested a combination of Grammatical Evolution
with PSO \cite{ge_mainpaper}. Also Pan et al \cite{siman_pso} suggested
a method that combines the PSO method with a variation of the simulated
annealing. Likewise, Mughal et al \cite{simman_pso_energy}, proposed
a hybrid technique that effectively combines PSO and simulated annealing
for solar energy problems. In the same way, Lin et al \cite{pso_de}
...
iterations of Particle Swarm Optimization \cite{pso_local1,pso_local2}.
Also, Gao et al suggested the usage of a PSO method to reduce energy
consumption \cite{psoEnergyGao}. Of course, these techniques significantly
increase the computational time required by the method, but also significantly
...
Olen¨ek et al\cite{parallel_go1} proposed an asynchronous parallel
...
Also, Regis and Shoemaker \cite{paralleGo2} employed Radial Basis
Functions \cite{rbf1} for parallel global optimization. A systematic
review of parallel metaheuristics can be found in the work of Alba
et al \cite{parallelReview1}. Also, a review of metaheuristics that
are implemented on modern GPU architectures can be found in the work
of Essaid et al \cite{parallelReview2}. The Particle Swarm Optimization
...
parallel asynchronous implementation of the PSO algorithm \cite{ppso1},
Tewolde et al \cite{ppso2} proposed a Multi - swarm PSO algorithm
where the swarm is divided into a set of subswarms that are executed
in parallel. Also, Ouyang et al \cite{ppso3} proposed a parallel
...
\cite{ppso4} investigated the impact of communication strategies
...
work of Lalwani et al \cite{parallelPsoReview}.
...
by Eberhart and Shi \cite{RandomInertia} and it is based on random
values. Also, the proposed stopping rule is based on the work of Charilogis
and Tsoulos \cite{ipso}, giving excellent results when applied on
...
The main disadvantages of the PSO method are its tendency to converge
quickly to a local minimum in some complex or high-dimensional objective
functions, and its performance heavily depends on the initial particle
distribution, as incorrect values may lead to suboptimal solutions.
The proposed method overcomes these issues and aims to improve the
algorithm's performance in terms of speed and reduced consumption
of computational resources. In general, any optimization method can
be parallelized, ensuring the ability to utilize fewer computational
resources and saving time in the resolution process.
...
the global minimum of the objective function. The formulation of the
PSO algorithm is shown in Algorithm \ref{alg:psoSerial}. The approach
of the underlying algorithm is based on representing the problem as
a swarm of particles, where each particle represents a candidate solution
in the Euclidean search space. During the optimization process, the
particles move in the search space based on their individual experience
and also on social interactions. Using these two influences, the particles
adjust their velocities and positions during the iterations of the
algorithm. The parallel version of the underlying article also promotes
social interactions among sub-swarms using different propagation schemes.
...
and Eberhart \cite{pso_major}. In the current article a simple inertia
...
of Powell \cite{Powell} was used a local search procedure.
\end{enumerate}
\end{enumerate}
\end{algorithm}
...
These components will be discussed in the following subsections. The
overall algorithm is graphically outlined in Figure \ref{fig:The-overall-algorithm.}.

\begin{figure}[H]
\includegraphics[scale=0.4]{ppso_datagram}

\caption{The overall algorithm.\label{fig:The-overall-algorithm.}}
...
were used\cite{Ali1,Floudas1}. These objective functions have been
extensively studied by many researchers in many publications\cite{testfunctions1,testfunctions2,testfunctions3,testfunctions4}.
...
\hline 
\textbf{Gkls}\cite{gkls} & {\footnotesize{}$f(x)=\mbox{Gkls}(x,n,w)$} & {\footnotesize{}$n=2,3$}\tabularnewline
\hline 
...
\hline 
\textbf{Potential}\cite{Jones} & {\footnotesize{}$V_{LJ}(r)=4\epsilon\left[\left(\frac{\sigma}{r}\right)^{12}-\left(\frac{\sigma}{r}\right)^{6}\right]$} & {\footnotesize{}$n=3,5$}\tabularnewline
\hline 
...
\cite{openmp} was utilized. The programming language used was ANSI
...
technique, i.e. running on a single processor, was compared with the
folllowing widely used global optimization techniques: 
\begin{enumerate}
\item A genetic algorithm, denoted as GENETIC in the the following table.
\item A simple version of Particle Swarm Optimization, denoted as PSO in
the following table.
\item The parallel implementation of the GaLib library \cite{GaLib}, denoted
as GALIB in the experimental results.
\item The Parallel Global Best-Worst Particle Swarm Optimization Algorithm
\cite{PGBWPSO}, denoted as PGBWPSO.
\end{enumerate}
In all cases the same number of particles or chromosomes and the same
maximum number of generations were used. In addition, after each method
was executed, the local optimization method BFGS was used to improve
the global minimum found. The results for this experiment are reported
...
\hline 
Function & GENETIC & PSO & GALIB & PGBWPSO & PROPOSED\tabularnewline
\hline 
\hline 
BF1 & 9581 & 19144(0.83) & 11802 & 1988(0.16) & 12625\tabularnewline
\hline 
BF2 & 10014 & 19121(0.90) & 11549 & 1980(0.36) & 13108\tabularnewline
\hline 
BRANIN & 9289 & 17760 & 9003 & 1780(0.93) & 8574\tabularnewline
\hline 
CIGAR10 & 40226 & 39553 & 28498(0.50) & 16649 & 40274\tabularnewline
\hline 
CM4 & 15360 & 22829 & 1513(0.70) & 9169(0.20) & 11512\tabularnewline
\hline 
DISCUS10 & 40216 & 22359 & 28798 & 11342 & 37848\tabularnewline
\hline 
EASOM & 9994 & 2897 & 35550 & 1381 & 4608\tabularnewline
\hline 
ELP10 & 40273 & 31192 & 29654 & 8993 & 23436\tabularnewline
\hline 
EXP4 & 14084 & 21375 & 8349 & 11033 & 9062\tabularnewline
\hline 
EXP16 & 40215 & 27755 & 8800 & 24885 & 22408\tabularnewline
\hline 
EXP64 & 40237 & 26155 & 9455 & 38800 & 40238\tabularnewline
\hline 
GKLS250 & 8361 & 16217 & 12645 & 1906(0.83) & 8070\tabularnewline
\hline 
GKLS350 & 12697(0.97) & 20393 & 15351(0.97) & 6093(0.80) & 10696(0.97)\tabularnewline
\hline 
GRIEWANK2 & 9298(0.97) & 20004(0.87) & 11344(0.67) & 1544(0.13) & 11064\tabularnewline
\hline 
POTENTIAL3 & 27799 & 20876 & 15456 & 13054 & 12876\tabularnewline
\hline 
POTENTIAL5 & 40240 & 25809 & 15902 & 13292 & 38377\tabularnewline
\hline 
HANSEN & 14951(0.93) & 16945 & 35011 & 1305(0.83) & 12467\tabularnewline
\hline 
HARTMAN3 & 11268 & 22259 & 35211 & 5916(0.96) & 10018\tabularnewline
\hline 
HARTMAN6 & 21396(0.63) & 33679(0.33) & 60457(0.60) & 11261(0.43) & 15082(0.53)\tabularnewline
\hline 
RASTRIGIN & 8967 & 16044 & 22753(0.97) & 2138(0.36) & 9286(0.93)\tabularnewline
\hline 
ROSENBROCK4 & 40233 & 26367 & 15701 & 11447 & 25120\tabularnewline
\hline 
ROSENBROCK8 & 40271 & 32750 & 18449 & 17212 & 38577\tabularnewline
\hline 
SHEKEL5 & 19403(0.70) & 29079(0.33) & 50453 & 8658(0.16) & 15409(0.43)\tabularnewline
\hline 
SHEKEL7 & 19376(0.80) & 27817(0.47) & 60791(0.77) & 7839(0.13) & 14989(0.63)\tabularnewline
\hline 
SHEKEL10 & 19829(0.77) & 26479(0.83) & 61450(0.77) & 7693(0.23) & 15087(0.67)\tabularnewline
\hline 
SINU4 & 15788 & 23915 & 60805 & 10367(0.43) & 12298\tabularnewline
\hline 
SINU8 & 30928 & 27834(0.97) & 90300 & 13307(0.56) & 15500\tabularnewline
\hline 
TEST2N4 & 17109 & 23983(0.97) & 20844 & 8625(0.70) & 14520(0.70)\tabularnewline
\hline 
TEST2N5 & 19464 & 30817 & 20751(0.87) & 9615(0.43) & 14801(0.47)\tabularnewline
\hline 
TEST2N6 & 24217 & 29067(0.90) & 20898(0.80) & 9185(0.20) & 17444(0.23)\tabularnewline
\hline 
TEST2N7 & 26824 & 32337(0.60) & 32553(0.60) & 10007(0.20) & 22780(0.23)\tabularnewline
\hline 
TEST30N3 & 17575 & 15660 & 16857 & 5061(0.96) & 7814\tabularnewline
\hline 
TEST30N4 & 17395 & 23519 & 16900 & 6983(0.96) & 8014\tabularnewline
\hline 
\textbf{TOTAL} & \textbf{732878(0.96)} & \textbf{791990(0.91)} & \textbf{894303(0.92)} & \textbf{310508(0.67)} & \textbf{573980(0.87)}\tabularnewline
\hline 
\end{tabular}
\end{table}
...
Moreover, although method PGBWPSO requires less total number of calls
than the proposed technique, the proposed technique is shown to significantly
outperform it in reliability of finding the global minimum.
...
was thoroughly tested also in the work of Sergeyev et al \cite{Sergeyev}.
...
was used to train an artificial neural network \cite{nn1,nn2} with
10 processing nodes applied on two widely used classification problems
from the relevant literature: the Wine problem \cite{wine1,wine2}
and the Parkinsons problem \cite{parkinsons}. The results are graphically
outlined in Figure \ref{fig:mlpTime}.

\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.8]{mlp_time.eps.png}
\par\end{centering}
\caption{Total execution time for different number of parallel processing threads
...
calls to find each global minimum. Overall, the main contributions
of the proposed method are: the velocity calculation mechanism, the
propagation among parallel computing units, and the termination rule
represents an innovative technique in improving the performance of
the algorithm. 
...
\bibitem{dynamicMethod}M. Yassami, P.A. Ashtari, novel hybrid optimization
algorithm: Dynamic hybrid optimization algorithm, Multimedia Tools
...
\bibitem{psoEnergyGao}Z. Gao, J. Yu, A. Zhao, Q. Hu, S. Yang, Optimal
chiller loading by improved parallel particle swarm optimization algorithm
for reducing energy consumption, International Journal of Refrigeration
\textbf{136}, pp. 61-70, 2022.

\bibitem{parallel_go1}J. Olen¨ek, T. Tuma, J. Puhan, Á B\H{u}rmen,
A new asynchronous parallel global optimization method based on simulated
...
\bibitem{PGBWPSO}L. Kumar, M. Pandey, M.K. Ahirwal, Parallel Global
Best-Worst Particle Swarm Optimization Algorithm for solving optimization
problems, Applied Soft Computing \textbf{142}, 110329, 2023.
